...
const applyCoupon=async(req,res)=>{
    try {
        ...
        console.log(coupon)
        if(!coupon){
            return res.json({success:false,message:"Inavalid or inactive coupon."});
        }
        ...
        const cartTotal=userCart.reduce((sum,item)=>sum+=item.productId.salesPrice*item.quantity,0);
