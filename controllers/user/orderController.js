...
const orderPlaced = async (req, res) => {
  try {
    ...
    if (
      ...
      typeof req.session.appliedCoupon.discount === "number"
    ) {
...
const downloadInvoice = async (req, res, next) => {
  ...
  try {
    ...
    doc.fontSize(36).font("Helvetica-Bold").text("INVOICE", 40, 80);
    ...
    doc.font("Helvetica-Bold").fontSize(14).text("INVOICE TO:", 40, 150);
    ...
    doc
      .fontSize(12)
      .text(`Order ID: ${orderId}`, doc.page.width - 400, 170, {
        align: "right",
      })
      .text(
        `Date: ${new Date(orderDetails.createdOn).toLocaleDateString()}`,
        doc.page.width - 200,
        190,
        { align: "right" }
      );
    ...
    doc
      ...
      .fontSize(12)
      .text(
        `Payment Method: ${orderDetails.payment.method === "cod" ? "Cash on Delivery" : orderDetails.payment.method === "razorpay" ? "Online" : "Wallet"}`,
        40,
        yPosition + 40
      )
      .text(
        `Payment Status: ${orderDetails.payment.status}`,
        40,
        yPosition + 60
      );
    ...
    doc
      ...
      .text(
        `Rs.${orderDetails?.couponUsed?.couponDiscount || "0"}`,
        500,
        yPosition + 40
      )
...
const cancelOrder = async (req, res) => {
  try {
    ...
    const itemIndex = findOrder.orderedItems.findIndex(
    ...
    if (
      findOrder.payment.method === "razorpay" ||
      findOrder.payment.method === "wallet"
    ) {
...
const returnOrder = async (req, res) => {
  ...
  try {
    ...
    if (
      order.orderedItems.some((item) => item.productStatus === "Return Request")
    ) {
