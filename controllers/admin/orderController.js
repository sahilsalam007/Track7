  ...
  try {
    ...
    if (status === "Delivered") {
      ...
      order.orderedItems.forEach((item) => {
        if (
          !["Cancelled", "Return Request", "Returned"].includes(
            item.productStatus
          )
        ) {
...
const approveItemReturn = async (req, res) => {
  try {
    ...
    console.log(
      `Item [${itemId}] in Order [${order.orderId}] approved for return`
    );

    return res
      .status(200)
      .json({ success: true, message: "Item return approved successfully" });
  } catch (error) {
    console.error("Approve item return error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Error processing return approval" });
  }
};
