...
const getSalesReport = async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      ...
      errors.push(
        "Please provide both startDate and endDate for custom filter."
      );
      return res.render("sales-report", {
        errors,
        data: [],
    ...
    const salesMetrics = allOrders.reduce(
      (acc, order) => ({
...
const downloadPDF = async (req, res) => {
  try {
    ...
    const orderDetails = orders.map((order) => {
      const tax = +(order.finalAmount * 0.1).toFixed(2);
      return {
    ...
    doc
      .fontSize(12)
      .font("Helvetica-Bold")
      .text("Sales Summary", 45, summaryStartY + 5, { underline: true });
    doc
      .fontSize(11)
      .font("Helvetica")
    ...
    doc
      .fontSize(11)
      .fillColor("#333")
      .text(`Report Date: ${today.toDateString()}`, 400, summaryStartY + 5, {
        align: "right",
      });
    ...
    orderDetails.forEach((order) => {
      ...
      doc.text(order.orderId.slice(0, 15) + "...", 40, rowTop + 7, {
        width: 90,
      });
      doc.text(order.orderDate.toLocaleDateString(), 130, rowTop + 7, {
        width: 80,
      });
